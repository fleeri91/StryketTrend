...
import { Distribution, Odds } from 'types/Stryktipset'
...
const EventItem = ({ startTime, teams, distribution, odds, event, onClick }: EventItemProps): JSX.Element => {
                ...
                deltaType={
                  event?.distribution[getIndexToRender(event.distribution)].home
                    ? getDistributionDeltaType(
                        Number(event?.distribution[getIndexToRender(event.distribution)].home),
                        Number(distribution.home)
                      )
                    : 'unchanged'
                }
                tooltip={event && `${event?.distribution[getIndexToRender(event.distribution)].home} %`}
              ...
              <BadgeDelta
                ...
                deltaType={
                  event?.distribution[getIndexToRender(event.distribution)].draw
                    ? getDistributionDeltaType(
                        Number(event?.distribution[getIndexToRender(event.distribution)].draw),
                        Number(distribution.draw)
                      )
                    : 'unchanged'
                }
                tooltip={event && `${event?.distribution[getIndexToRender(event.distribution)].draw} %`}
              ...
              <BadgeDelta
                ...
                deltaType={
                  event?.distribution[getIndexToRender(event.distribution)].away
                    ? getDistributionDeltaType(
                        Number(event?.distribution[getIndexToRender(event.distribution)].away),
                        Number(distribution.away)
                      )
                    : 'unchanged'
                }
                tooltip={event && `${event?.distribution[getIndexToRender(event.distribution)].away} %`}
                ...
                <BadgeDelta
                  ...
                  deltaType={
                    event?.odds[getIndexToRender(event.odds)].home
                      ? getOddsDeltaType(
                          parseFloat(event?.odds[getIndexToRender(event.odds)].home.replace(',', '.')),
                          parseFloat(odds.home.replace(',', '.'))
                        )
                      : 'unchanged'
                  }
                  tooltip={event && event?.odds[getIndexToRender(event.odds)].home}
                ...
                <BadgeDelta
                  ...
                  deltaType={
                    event?.odds[getIndexToRender(event.odds)].draw
                      ? getOddsDeltaType(
                          parseFloat(event?.odds[getIndexToRender(event.odds)].draw.replace(',', '.')),
                          parseFloat(odds.draw.replace(',', '.'))
                        )
                      : 'unchanged'
                  }
                  tooltip={event && event?.odds[getIndexToRender(event.odds)].draw}
                ...
                <BadgeDelta
                  ...
                  deltaType={
                    event?.odds[getIndexToRender(event.odds)].away
                      ? getOddsDeltaType(
                          parseFloat(event?.odds[getIndexToRender(event.odds)].away.replace(',', '.')),
                          parseFloat(odds.away.replace(',', '.'))
                        )
                      : 'unchanged'
                  }
                  tooltip={event && event?.odds[getIndexToRender(event.odds)].away}
